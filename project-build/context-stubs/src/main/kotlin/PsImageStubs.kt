...
private val LABESL_PS = mutableListOf(PsLabel("1", "2", "3"))
...
fun stubReadToTransport(): Pair<PsBeContext, ImageReadResponse> {
    return Pair ( PsBeContext (
        ...
        state = PsState.FINISHING,
        response = PsImageStubsItems.FULL_PSIMAGE
    ),
        ImageReadResponse (
            result = ResponseResult.SUCCESS,
            image = PsImageStubsItems.FULL_IMAGE
        )
    )
}
...
fun stubSearchToTransport(): Pair<PsBeContext, ImageSearchResponse> {
    return Pair(
        PsBeContext(
            ...
            state = PsState.FINISHING,
            responseList = mutableListOf(PsImageStubsItems.FULL_PSIMAGE, PsImageStubsItems.FULL_PSIMAGE)
        ),
        ImageSearchResponse(
            result = ResponseResult.SUCCESS,
            list = listOf(PsImageStubsItems.FULL_IMAGE, PsImageStubsItems.FULL_IMAGE)
        )
    )
}
